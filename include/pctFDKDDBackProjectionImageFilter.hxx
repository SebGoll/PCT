...
void
FDKDDBackProjectionImageFilter<TInputImage, TOutputImage>::DynamicThreadedGenerateData(
  const OutputImageRegionType & outputRegionForThread)
{
  ...
  for (unsigned int iProj = iFirstProj; iProj < iFirstProj + nProj; iProj++)
  {
    ...
    if (sdd == 0.)
    {
      ...
      matProjTrans = rtk::ThreeDCircularProjectionGeometry::ComputeTranslationHomogeneousMatrix(
        sourceOffsetX - projOffsetX, sourceOffsetY - projOffsetY, 0.);
      // Flip because disance-driven binning puts the source on the negative side and the backprojection on the positive
      // side
      matProjTrans[2][2] = -1.;
      ...
      matrixVol = matrixStackProj.GetVnlMatrix() * matProjTrans.GetVnlMatrix() *
                  geometry->GetSourceTranslationMatrices()[iProj].GetVnlMatrix() *
                  geometry->GetRotationMatrices()[iProj].GetVnlMatrix() * matrixVol.GetVnlMatrix();
    ...
    while (!itOut.IsAtEnd())
    {
      ...
      else // Cone-beam
      {
        ...
        // Distance driven
        pointProj[Dimension - 1] = (sid / perspFactor - sid - m_ProjectionStack->GetOrigin()[Dimension - 1]) /
                                   m_ProjectionStack->GetSpacing()[Dimension - 1];
        // Interpolate if in projection
        if (interpolator->IsInsideBuffer(pointProj))
        {
...
FDKDDBackProjectionImageFilter<TInputImage, TOutputImage>::GetDDProjection(const unsigned int iProj)
{
...
FDKDDBackProjectionImageFilter<TInputImage, TOutputImage>::GetIndexToIndexProjectionMatrix(
  const unsigned int          iProj,
  const ProjectionImageType * proj)
{
  const unsigned int Dimension = TInputImage::ImageDimension;
  ...
  return ProjectionMatrixType(matrixProj.GetVnlMatrix() * this->GetGeometry()->GetMatrices()[iProj].GetVnlMatrix() *
