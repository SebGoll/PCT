...
template <typename TImage>
void
SmallHoleFiller<TImage>::SetHolePixel(typename TImage::PixelType pixel)
{
...
template <typename TImage>
void
SmallHoleFiller<TImage>::Iterate()
{
  // Make a copy of the current intermediate output. We use this to determine which pixels were holes at the beginning
  // of this iteration.
  ...
  // Do not mark pixels filled on this iteration as known. This will result in a less accurate filling, as it favors the
  // colors of pixels that occur earlier in the raster scan order.
  typename TImage::SizeType radius;
  radius.Fill(1);

  itk::NeighborhoodIterator<TImage> intermediateNeighborhoodIterator(
    radius, temporaryImage, temporaryImage->GetLargestPossibleRegion());
  ...
  while (!intermediateNeighborhoodIterator.IsAtEnd())
  {
    if (intermediateNeighborhoodIterator.GetCenterPixel() == this->HolePixel) // We want to fill this pixel
    {
      ...
      if (validPixels > 0)
      {
        ...
        typename TImage::PixelType pixelAverage = static_cast<typename TImage::PixelType>(
          pixelSum *
          (1.0 / validPixels)); // We multiply by the reciprocal because operator/ is not defined for all types.
...
template <typename TImage>
bool
SmallHoleFiller<TImage>::HasEmptyPixels()
{
